...
linkspeed_bps                   FatTreeTopology::_asymmetric_downlink_speeds[] = {0, 0, 0};
FatTreeTopology::AsymmetricMode FatTreeTopology::_asymmetric_mode[]            = {
    FatTreeTopology::SYM, FatTreeTopology::SYM, FatTreeTopology::SYM};
uint32_t FatTreeTopology::_single_asymmetric_link[]      = {0, 0, 0};
uint32_t FatTreeTopology::_asymmetric_number_of_switches = 1;
...
double   FatTreeTopology::_failed_link_ratio          = 0.2;
bool FatTreeTopology::_failed_bidirectional      = false;
bool     FatTreeTopology::full_fail                     = false;
int     FatTreeTopology::_num_to_fail                  = 0;

void FatTreeTopology::set_tier_parameters(int           tier,
                                          int           radix_up,
                                          int           radix_down,
                                          mem_b         queue_up,
                                          mem_b         queue_down,
    ...
    _downlink_speeds[tier] = linkspeed;  // this is the link going downwards from this tier. up/down
                                         // linkspeeds are symmetric.
    _oversub[tier] = oversub;
    // xxx what to do about queue sizes
}

// load a config file and use it to create a FatTreeTopology
FatTreeTopology* FatTreeTopology::load(const char*         filename,
    ...
    if (file.is_open()) {
        FatTreeTopology* ft =
            load(file, logger_factory, eventlist, queuesize, q_type, sender_q_type);
        file.close();
        return ft;
    } else {
...
FatTreeTopology* FatTreeTopology::load(istream&            file,
                                       QueueLoggerFactory* logger_factory,
                                       EventList&          eventlist,
                                       mem_b               queuesize,
                                       queue_type          q_type,
    ...
    do {
        ...
        if (tokens.size() == 0 || tokens[0][0] == '#') {
            ...
            if (_downlink_speeds[current_tier] != 0) {
                cerr << "Duplicate linkspeed setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            _downlink_speeds[current_tier] = ((linkspeed_bps)stoi(tokens[1])) * 1000000000;
        } else if (tokens[0] == "asymmetric_speed_gbps") {
            if (_asymmetric_downlink_speeds[current_tier] != 0) {
                cerr << "Duplicate linkspeed setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            _asymmetric_downlink_speeds[current_tier] =
                ((linkspeed_bps)stoi(tokens[1])) * 1000000000;
        } else if (tokens[0] == "asymmetric_mode") {
            if (_asymmetric_mode[current_tier] != FatTreeTopology::SYM) {
                cerr << "Duplicate linkspeed setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            if (tokens[1] == "all") {
                _asymmetric_mode[current_tier] = FatTreeTopology::ALL;
            } else if (tokens[1] == "single") {
                _asymmetric_mode[current_tier] = FatTreeTopology::SINGLE;
            }
        } else if (tokens[0] == "asymmetric_number_of_switches") {
            _asymmetric_number_of_switches = stoi(tokens[1]);
        } else if (tokens[0] == "radix_up") {
            if (_radix_up[current_tier] != 0) {
                cerr << "Duplicate radix_up setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            if (current_tier == 2) {
                cerr << "Can't specific radix_up for tier " << current_tier << " at line "
                     << linecount << " (no uplinks from top tier!)" << endl;
                exit(1);
            }
            _radix_up[current_tier] = stoi(tokens[1]);
        } else if (tokens[0] == "radix_down") {
            if (_radix_down[current_tier] != 0) {
                cerr << "Duplicate radix_down setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            ...
            if (_queue_up[current_tier] != 0) {
                cerr << "Duplicate queue_up setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            if (current_tier == 2) {
                cerr << "Can't specific queue_up for tier " << current_tier << " at line "
                     << linecount << " (no uplinks from top tier!)" << endl;
                exit(1);
            }
            ...
            if (_queue_down[current_tier] != 0) {
                cerr << "Duplicate queue_down setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            ...
            if (_oversub[current_tier] != 1) {
                cerr << "Duplicate oversubscribed setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            ...
            if (_bundlesize[current_tier] != 1) {
                cerr << "Duplicate bundle size setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            ...
            if (_switch_latencies[current_tier] != 0) {
                cerr << "Duplicate switch_latency setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            ...
            if (_link_latencies[current_tier] != 0) {
                cerr << "Duplicate link latency setting for tier " << current_tier << " at line "
                     << linecount << endl;
                exit(1);
            }
            ...
            cerr << "Allowed attributes are: tier, downlink_speed_gbps, radix_up, radix_down, "
                    "queue_up, queue_down, oversubscribed, bundle, switch_latency_ns, "
                    "downlink_latency_ns"
                 << endl;
            exit(1);
        }
    ...
    for (uint32_t tier = 0; tier < _tiers; tier++) {
        ...
        if (_asymmetric_mode[tier] == FatTreeTopology::SINGLE) {
            _single_asymmetric_link[tier] =
                rand() % _radix_down[tier];  // Not really random but should be enough
        }
    }

    cout << "Topology load done\n";
    FatTreeTopology* ft = new FatTreeTopology(
        no_of_nodes, 0, 0, logger_factory, &eventlist, q_type, 0, 0, sender_q_type);
    cout << "FatTree constructor done, " << ft->no_of_nodes() << " nodes created\n";
    return ft;
}

FatTreeTopology::FatTreeTopology(uint32_t            no_of_nodes,
                                 linkspeed_bps       linkspeed,
                                 mem_b               queuesize,
                                 QueueLoggerFactory* logger_factory,
                                 EventList*          ev,
    ...
    if ((latency != 0 || switch_latency != 0)) {
        for (int tier = TOR_TIER; tier <= CORE_TIER; tier++) {
            if ((_link_latencies[tier] != 0 && _link_latencies[tier] != latency) ||
                ...
                cerr << "Tier " << tier << " Link latency " << _link_latencies[tier]
                     << " Switch Latency " << _switch_latencies[tier] << endl;
                cerr << "Global " << " Latency " << latency << " Switch Latency " << switch_latency
                     << endl;
                cerr << "Don't set latencies using both the constructor and set_latencies - use "
                        "only one of the two\n";
                exit(1);
            }
        }
    ...
    if (_link_latencies[TOR_TIER] == 0) {
        ...
        for (uint32_t i = 0; i < _tiers; i++) {
            _link_latencies[i] = _hop_latency;
        }
    } else {
        ...
        if (_tiers == 3) {
            _diameter_latency += 2 * _link_latencies[CORE_TIER] + _switch_latencies[AGG_TIER] +
                                 _switch_latencies[CORE_TIER];
        }

        cout << "Fat Tree topology (0) with " << timeAsUs(_link_latencies[TOR_TIER])
             << "us Src-ToR links, " << timeAsUs(_link_latencies[AGG_TIER]) << "us ToR-Agg links, ";
        if (_tiers == 3) {
        ...
        if (_tiers == 3) {
            cout << ", " << timeAsUs(_switch_latencies[CORE_TIER]) << "us Core switch latency."
                 << endl;
        }
...
FatTreeTopology::FatTreeTopology(uint32_t            no_of_nodes,
    ...
    _diameter_latency = 2 * (_hop_latency + _hop_latency) + 3 * _switch_latency;
    if (_tiers == 3) {
        _diameter_latency += 2 * _hop_latency + 2 * _switch_latency;
    }

    cout << "Fat tree topology (1) with " << no_of_nodes << " nodes" << " and "
         << timeAsUs(_diameter_latency) << "us diameter latency." << endl;
    ;
    set_params(no_of_nodes);
...
FatTreeTopology::FatTreeTopology(uint32_t            no_of_nodes,
    ...
    cout << "Fat tree topology (2) with " << no_of_nodes << " nodes" << " and "
         << timeAsUs(_diameter_latency) << "us diameter latency." << endl;
    ;
    set_params(no_of_nodes);
...
FatTreeTopology::FatTreeTopology(uint32_t            no_of_nodes,
    ...
    cout << "Fat tree topology (3) with " << no_of_nodes << " nodes" << " and "
         << timeAsUs(_diameter_latency) << "us diameter latency." << endl;
    ;
    set_params(no_of_nodes);
...
void FatTreeTopology::set_linkspeeds(linkspeed_bps linkspeed) {
    if (linkspeed != 0 && _downlink_speeds[TOR_TIER] != 0 &&
        linkspeed != _downlink_speeds[TOR_TIER]) {
        cerr << "Don't set linkspeeds using both the constructor and set_tier_parameters - use "
                "only one of the two\n";
        exit(1);
    }
    if (linkspeed == 0 && _downlink_speeds[TOR_TIER] == 0) {
...
void FatTreeTopology::set_custom_params(uint32_t no_of_nodes) {
    ...
    for (uint32_t tier = TOR_TIER; tier < _tiers; tier++) {
        ...
        if (_radix_down[tier] % _bundlesize[tier] != 0) {
            cerr << "Mismatch between tier " << tier << " down radix of " << _radix_down[tier]
                 << " and bundlesize " << _bundlesize[tier] << "\n";
        ...
        if (tier < (_tiers - 1) && _radix_up[tier] % _bundlesize[tier + 1] != 0) {
            cerr << "Mismatch between tier " << tier << " up radix of " << _radix_up[tier]
                 << " and tier " << tier + 1 << " down bundlesize " << _bundlesize[tier + 1]
                 << "\n";
            cerr << "Radix must be a multiple of bundlesize\n";
            exit(1);
        }
    ...
    if (_hosts_per_pod % _radix_down[TOR_TIER] != 0) {
        cerr << "Mismatch between TOR radix " << _radix_down[TOR_TIER] << " and podsize "
             << _hosts_per_pod << endl;
        exit(1);
    }
    _tor_switches_per_pod = _hosts_per_pod / _radix_down[TOR_TIER];

    assert((no_of_nodes * _downlink_speeds[TOR_TIER]) %
               (_downlink_speeds[AGG_TIER] * _oversub[TOR_TIER]) ==
           0);
    no_of_tor_uplinks = (no_of_nodes * _downlink_speeds[TOR_TIER]) /
                        (_downlink_speeds[AGG_TIER] * _oversub[TOR_TIER]);
    cout << "no_of_tor_uplinks: " << no_of_tor_uplinks << endl;

    if (_radix_down[TOR_TIER] / _radix_up[TOR_TIER] != _oversub[TOR_TIER]) {
        cerr << "Mismatch between TOR linkspeeds (" << speedAsGbps(_downlink_speeds[TOR_TIER])
             << "Gbps down, " << speedAsGbps(_downlink_speeds[AGG_TIER])
             << "Gbps up) and TOR radix (" << _radix_down[TOR_TIER] << " down, "
             << _radix_up[TOR_TIER] << " up) and oversubscription ratio of " << _oversub[TOR_TIER]
             << endl;
        exit(1);
    }
    ...
    if (_agg_switches_per_pod * _bundlesize[AGG_TIER] != _radix_up[TOR_TIER]) {
        cerr << "Mismatch between TOR up radix " << _radix_up[TOR_TIER] << " and "
             << _agg_switches_per_pod << " aggregation switches per pod required by "
             << no_of_tor_uplinks << " TOR uplinks in " << no_of_pods << " pods "
        ...
        if (_bundlesize[AGG_TIER] == 1 && _radix_up[TOR_TIER] % _agg_switches_per_pod == 0 &&
            ...
            cerr << "Did you miss specifying a Tier 1 bundle size of "
    ...
    if (_tiers == 3) {
        assert((no_of_tor_uplinks * _downlink_speeds[AGG_TIER]) %
                   (_downlink_speeds[CORE_TIER] * _oversub[AGG_TIER]) ==
               0);
        no_of_agg_uplinks = (no_of_tor_uplinks * _downlink_speeds[AGG_TIER]) /
                            (_downlink_speeds[CORE_TIER] * _oversub[AGG_TIER]);
        ...
        if (no_of_core_switches % _agg_switches_per_pod != 0) {
            cerr << "Topology results in " << no_of_core_switches
                 ...
                 << " aggregation switches per pod, computed from Tier 0 and 1 values\n";
            exit(1);
        }

        if ((no_of_core_switches * _bundlesize[CORE_TIER]) / _agg_switches_per_pod !=
            _radix_up[AGG_TIER]) {
            cerr << "Mismatch between the AGG switch up-radix of " << _radix_up[AGG_TIER]
                 << " and calculated " << _agg_switches_per_pod
                 << " aggregation switched per pod with " << no_of_core_switches << " core switches"
                 << endl;
            if (_bundlesize[CORE_TIER] == 1 &&
                ...
                cerr << "Did you miss specifying a Tier 2 bundle size of "
                     << _radix_up[AGG_TIER] / (no_of_core_switches / _agg_switches_per_pod) << "?"
                     << endl;
            } else if (_radix_up[AGG_TIER] % (no_of_core_switches / _agg_switches_per_pod) == 0 &&
                       _radix_up[AGG_TIER] / (no_of_core_switches / _agg_switches_per_pod) !=
                           _bundlesize[CORE_TIER]) {
                cerr << "Tier 2 bundle size is " << _bundlesize[CORE_TIER]
    ...
    total_tor_uplinks = no_of_tor_uplinks;
...
void FatTreeTopology::set_params(uint32_t no_of_nodes) {
    if (_hosts_per_pod > 0) {
        // if we've set all the detailed parameters, we'll use them, otherwise fall through to
        // defaults
...
void FatTreeTopology::alloc_vectors() {
    ...
    if (_tiers == 3) {
        ...
        pipes_nc_nup.resize(NCORE,
                            vector<vector<Pipe*>>(NAGG, vector<Pipe*>(_bundlesize[CORE_TIER])));
    ...
    if (_tiers == 3) {
        pipes_nup_nc.resize(NAGG,
                            vector<vector<Pipe*>>(NCORE, vector<Pipe*>(_bundlesize[CORE_TIER])));
...
BaseQueue* FatTreeTopology::alloc_src_queue(QueueLogger* queueLogger, linkspeed_bps linkspeed) {
    switch (_sender_qt) {
        case PRIORITY:
            return new PriorityQueue(
                linkspeed, memFromPkt(FEEDER_BUFFER), *_eventlist, queueLogger);
        case FAIR_PRIO:
...
BaseQueue* FatTreeTopology::asymmetric_alloc_queue(QueueLogger*   queueLogger,
                                                   mem_b          queuesize,
                                                   link_direction dir,
                                                   int            switch_tier,
                                                   bool           tor = false) {
    return alloc_queue(queueLogger,
                       _asymmetric_downlink_speeds[switch_tier],
                       queuesize,
                       dir,
                       switch_tier,
                       tor,
...
BaseQueue* FatTreeTopology::alloc_queue(QueueLogger*   queueLogger,
                                        ...
                                        bool           tor = false) {
    if (dir == UPLINK) {
        switch_tier++;  // _downlink_speeds is set for the downlinks, so uplinks need to use the
                        // tier above's linkspeed
    }
    return alloc_queue(
        queueLogger, _downlink_speeds[switch_tier], queuesize, dir, switch_tier, tor, false);
}
    ...
    switch (_qt) {
        case RANDOM:
            return new RandomQueue(
                speed, queuesize, *_eventlist, queueLogger, memFromPkt(RANDOM_BUFFER));
        case COMPOSITE: {
            CompositeQueue* q = new CompositeQueue(speed,
                                                   ...
                                                   *_eventlist,
                                                   ...
                                                   FatTreeSwitch::_disable_trim,
                                                   FatTreeSwitch::_low_priority_trim,
                                                   FatTreeSwitch::_no_droping_low_header,
                                                   FatTreeSwitch::_switch_random_drop_prob);

            if (_enable_ecn) {
                if (!tor || dir == UPLINK || _enable_ecn_on_tor_downlink) {
                    // don't use ECN on ToR downlinks unless configured so.
                    if (reduced_speed)
                        q->set_ecn_thresholds(_ecn_low * _failed_link_ratio,
                                              _ecn_high * _failed_link_ratio);
                    else
        ...
        case AEOLUS:
            return new AeolusQueue(speed,
                                   queuesize,
                                   FatTreeSwitch::_speculative_threshold_fraction * queuesize,
                                   *_eventlist,
                                   queueLogger);
        case AEOLUS_ECN: {
            AeolusQueue* q =
                new AeolusQueue(speed,
                                queuesize,
                                FatTreeSwitch::_speculative_threshold_fraction * queuesize,
                                *_eventlist,
                                queueLogger);
        ...
        case COMPOSITE_ECN:
            if (tor && dir == DOWNLINK)
                return new CompositeQueue(speed,
                                          queuesize,
                                          *_eventlist,
                                          queueLogger,
                                          FatTreeSwitch::_trim_size,
                ...
                return new ECNQueue(
                    speed, memFromPkt(2 * SWITCH_BUFFER), *_eventlist, queueLogger, memFromPkt(15));
...
void FatTreeTopology::init_network() {
    QueueLogger* down_queueLogger;
    QueueLogger* up_queueLogger;
    if (_tiers == 3) {
    ...
    for (uint32_t j = 0; j < NTOR; j++) {
        ...
        switches_lp[j] = new FatTreeSwitch(*_eventlist,
                                           fmt::format("Switch_LowerPod_{}", j),
                                           FatTreeSwitch::TOR,
                                           j,
                                           switch_latency,
                                           this);
    ...
    for (uint32_t j = 0; j < NAGG; j++) {
        ...
        switches_up[j] = new FatTreeSwitch(*_eventlist,
                                           fmt::format("Switch_UpperPod_{}", j),
                                           FatTreeSwitch::AGG,
                                           j,
                                           switch_latency,
                                           this);
    ...
    for (uint32_t j = 0; j < NCORE; j++) {
        ...
        switches_c[j] = new FatTreeSwitch(*_eventlist,
                                          fmt::format("Switch_Core_{}", j),
                                          FatTreeSwitch::CORE,
                                          j,
                                          switch_latency,
                                          this);
    ...
    for (uint32_t tor = 0; tor < NTOR; tor++) {
        ...
        for (uint32_t l = 0; l < link_bundles; l++) {
            ...
            for (uint32_t b = 0; b < _bundlesize[TOR_TIER]; b++) {
                if (_logger_factory) {
                    down_queueLogger = _logger_factory->createQueueLogger();
                    up_queueLogger   = _logger_factory->createQueueLogger();
                } else {
                    down_queueLogger = NULL;
                    up_queueLogger   = NULL;
                }
                if (_asymmetric_mode[TOR_TIER] == FatTreeTopology::SYM ||
                    tor >= _asymmetric_number_of_switches ||
                    (_asymmetric_mode[TOR_TIER] == FatTreeTopology::SINGLE &&
                     _single_asymmetric_link[TOR_TIER] != (_bundlesize[TOR_TIER] * l + b))) {
                    queues_nlp_ns[tor][srv][b] = alloc_queue(
                        down_queueLogger, _queue_down[TOR_TIER], DOWNLINK, TOR_TIER, true);
                    queues_ns_nlp[srv][tor][b] = alloc_src_queue(
                        up_queueLogger, _downlink_speeds[TOR_TIER]);  // linkspeeds are symmetric);
                } else {
                    queues_nlp_ns[tor][srv][b] = asymmetric_alloc_queue(
                        down_queueLogger, _queue_down[TOR_TIER], DOWNLINK, TOR_TIER, true);
                    queues_ns_nlp[srv][tor][b] = alloc_src_queue(
                        up_queueLogger,
                        _asymmetric_downlink_speeds[TOR_TIER]);  // linkspeeds are symmetric);
                }
                queues_nlp_ns[tor][srv][b]->setName(fmt::format("LS{}->DST{}({})", tor, srv, b));
                queues_ns_nlp[srv][tor][b]->setName(fmt::format("SRC{}->LS{}({})", srv, tor, b));
                ...
                simtime_picosec hop_latency =
                    (_hop_latency == 0) ? _link_latencies[TOR_TIER] : _hop_latency;
                pipes_nlp_ns[tor][srv][b] = new Pipe(hop_latency, *_eventlist);
                pipes_nlp_ns[tor][srv][b]->setName(
                    fmt::format("Pipe-LS{}->DST{}({})", tor, srv, b));
                ...
                if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN) {
                    ...
                    new LosslessInputQueue(
                        *_eventlist, queues_ns_nlp[srv][tor][b], switches_lp[tor], hop_latency);
                }

                pipes_ns_nlp[srv][tor][b] = new Pipe(hop_latency, *_eventlist);
                pipes_ns_nlp[srv][tor][b]->setName(
                    fmt::format("Pipe-SRC{}->LS{}({})", srv, tor, b));
            }
        }
    }


    std::vector<int> failing_list_tor;
    failing_list_tor.reserve(_num_to_fail);
    std::vector<char> used(total_tor_uplinks, 0);

    while (failing_list_tor.size() < _num_to_fail) {
        int x = rand() % total_tor_uplinks;
        if (!used[x]) {
            used[x] = 1;
            failing_list_tor.push_back(x);
        }
    } 

    int uplinks_per_switch = _radix_up[TOR_TIER];
    printf("Uplinks per switch: %d\n", uplinks_per_switch);

    // 2) Ensure each switch keeps at least one valid uplink
    int num_switches = (total_tor_uplinks + uplinks_per_switch - 1) / uplinks_per_switch;

    for (int s = 0; s < num_switches; ++s) {
        int start = s * uplinks_per_switch;
        int end   = std::min(start + uplinks_per_switch, total_tor_uplinks);
        int block_size = end - start;

        int cnt = 0;
        for (int i = start; i < end; ++i) cnt += used[i];

        if (cnt == block_size) {
            // All uplinks of this switch are failed -> un-fail one at random
            int idx = start + (rand() % block_size);
            used[idx] = 0;
        }
    }

    // 3) Rebuild the failed vector to reflect any un-failed corrections
    failing_list_tor.clear();
    for (int i = 0; i < total_tor_uplinks; ++i)
        if (used[i]) failing_list_tor.push_back(i);

    for (unsigned i = 0; i < failing_list_tor.size(); i++) {
        cout << "Failing TOR uplink " << failing_list_tor[i] << endl;
    }
    ...
    for (uint32_t tor = 0; tor < NTOR; tor++) {
        ...
        for (uint32_t agg = agg_min; agg <= agg_max; agg++) {
            for (uint32_t b = 0; b < _bundlesize[AGG_TIER]; b++) {
                ...
                if (_asymmetric_mode[AGG_TIER] == FatTreeTopology::SYM ||
                    agg - agg_min >= _asymmetric_number_of_switches ||
                    (_asymmetric_mode[AGG_TIER] == FatTreeTopology::SINGLE &&
                     _single_asymmetric_link[AGG_TIER] != (tor % _tor_switches_per_pod))) {
                    if (_tiers == 2 && (agg - agg_min) < _num_failed_links) {
                        queues_nup_nlp[agg][tor][b] = alloc_queue(down_queueLogger,
                                                                  _downlink_speeds[AGG_TIER],
                                                                  _queue_down[AGG_TIER],
                                                                  DOWNLINK,
                                                                  AGG_TIER,
                                                                  false,
                                                                  _failed_bidirectional);
                        cout << fmt::format("Failure: US{}->LS{}({}) - {}", agg, tor, b, agg_min)
                             << " linkspeed set to "
                             << speedAsGbps(_downlink_speeds[AGG_TIER] *
                                            (_failed_bidirectional ? _failed_link_ratio : 1.0))
                             << endl;

                        if (full_fail) {
                            queues_nup_nlp[agg][tor][b]->fail_queue = true;
                        }
                        
                        queues_nlp_nup[tor][agg][b] = alloc_queue(up_queueLogger,
                                                                  _downlink_speeds[AGG_TIER],
                                                                  _queue_up[TOR_TIER],
                                                                  UPLINK,
                                                                  TOR_TIER,
                                                                  true,
                                                                  true);
                        cout << fmt::format("Failure: LS{}->US{}({}) - {}", tor, agg, b, agg_min)
                             << " linkspeed set to "
                             << speedAsGbps(_downlink_speeds[AGG_TIER] * _failed_link_ratio)
                             << endl;
                    } else {

                        if (_num_failed_links == -1) {
                            _num_failed_links++;
                            printf("failing link US%d->LS%d(%d)\n", agg, tor, b);
                            alloc_queue(up_queueLogger,
                                                                  _downlink_speeds[AGG_TIER],
                                                                  _queue_up[TOR_TIER],
                                                                  UPLINK,
                                                                  TOR_TIER,
                            ...
                            queues_nlp_nup[tor][agg][b] = alloc_queue(
                            up_queueLogger, _queue_up[TOR_TIER], UPLINK, TOR_TIER, true);
                        }
                        queues_nup_nlp[agg][tor][b] = alloc_queue(
                            down_queueLogger, _queue_down[AGG_TIER], DOWNLINK, AGG_TIER);
                        
                    }
                } else {
                    if (_tiers == 2 && (agg - agg_min) < _num_failed_links) {
                        queues_nup_nlp[agg][tor][b] =
                            alloc_queue(down_queueLogger,
                                        _asymmetric_downlink_speeds[AGG_TIER],
                                        _queue_down[AGG_TIER],
                                        DOWNLINK,
                                        AGG_TIER,
                                        false,
                        ...
                        queues_nup_nlp[agg][tor][b] = asymmetric_alloc_queue(
                            down_queueLogger, _queue_down[AGG_TIER], DOWNLINK, AGG_TIER);
                        queues_nlp_nup[tor][agg][b] = asymmetric_alloc_queue(
                            up_queueLogger, _queue_up[TOR_TIER], UPLINK, TOR_TIER, true);
                    }
                }

                count_uplink++;
                bool is_failed_linear = false;
                for (unsigned idx = 0; idx < failing_list_tor.size(); idx++) {
                    if (failing_list_tor[idx] == count_uplink-1) {
                        is_failed_linear = true;
                        break;
                    }
                }

                if (is_failed_linear) {
                    cout << fmt::format("Total Failure: US{}->LS{}({})", agg, tor, b) << endl;
                    queues_nup_nlp[agg][tor][b]->fail_queue = true;
                } else {
                    //cout << fmt::format("Active Link: US{}->LS{}({})", agg, tor, b) << endl;
                }

                queues_nup_nlp[agg][tor][b]->setName(fmt::format("US{}->LS{}({})", agg, tor, b));
                queues_nlp_nup[tor][agg][b]->setName(fmt::format("LS{}->US{}({})", tor, agg, b));
                // if (logfile) logfile->writeName(*(queues_nup_nlp[agg][tor]));
                // cout << queues_nup_nlp[tor][agg][b]->str() << endl;
                // if (logfile) logfile->writeName(*(queues_nlp_nup[tor][agg]));

                simtime_picosec hop_latency =
                    (_hop_latency == 0) ? _link_latencies[AGG_TIER] : _hop_latency;
                pipes_nup_nlp[agg][tor][b] = new Pipe(hop_latency, *_eventlist);
                pipes_nup_nlp[agg][tor][b]->setName(
                    fmt::format("Pipe-US{}->LS{}({})", agg, tor, b));
                ...
                if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN) {
                    new LosslessInputQueue(
                        *_eventlist, queues_nlp_nup[tor][agg][b], switches_up[agg], hop_latency);
                    new LosslessInputQueue(
                        *_eventlist, queues_nup_nlp[agg][tor][b], switches_lp[tor], hop_latency);
                }

                pipes_nlp_nup[tor][agg][b] = new Pipe(hop_latency, *_eventlist);
                pipes_nlp_nup[tor][agg][b]->setName(
                    fmt::format("Pipe-LS{}->US{}({})", tor, agg, b));
        ...
        for (uint32_t agg = 0; agg < NAGG; agg++) {
            ...
            for (uint32_t l = 0; l < _radix_up[AGG_TIER] / _bundlesize[CORE_TIER]; l++) {
                ...
                for (uint32_t b = 0; b < _bundlesize[CORE_TIER]; b++) {
                    ...
                    if (_asymmetric_mode[CORE_TIER] == FatTreeTopology::SYM ||
                        core >= _asymmetric_number_of_switches ||
                        (_asymmetric_mode[CORE_TIER] == FatTreeTopology::SINGLE &&
                         _single_asymmetric_link[CORE_TIER] != (agg / _agg_switches_per_pod))) {
                        if ((l + agg * _agg_switches_per_pod) < failed_links) {
                            queues_nc_nup[core][agg][b] = alloc_queue(down_queueLogger,
                                                                      _downlink_speeds[CORE_TIER],
                                                                      _queue_down[CORE_TIER],
                                                                      DOWNLINK,
                                                                      CORE_TIER,
                                                                      false,
                                                                      true);
                            cout << fmt::format(
                                        "Adding link failure for agg_sw {} l {} b {}", agg, l, b)
                                 << endl;
                            queues_nup_nc[agg][core][b] = alloc_queue(up_queueLogger,
                                                                      _downlink_speeds[AGG_TIER],
                                                                      _queue_up[AGG_TIER],
                                                                      UPLINK,
                                                                      AGG_TIER,
                                                                      ...
                                                                      true);
                            if (full_fail) {
                                queues_nup_nc[agg][core][b]->fail_queue = true;
                            }
                        } else {
                            queues_nc_nup[core][agg][b] = alloc_queue(
                                down_queueLogger, _queue_down[CORE_TIER], DOWNLINK, CORE_TIER);
                            queues_nup_nc[agg][core][b] =
                                alloc_queue(up_queueLogger, _queue_up[AGG_TIER], UPLINK, AGG_TIER);
                        }
                    } else {
                        if ((l + agg * _agg_switches_per_pod) < failed_links) {
                            queues_nc_nup[core][agg][b] =
                                alloc_queue(down_queueLogger,
                                            _asymmetric_downlink_speeds[CORE_TIER],
                                            _queue_down[CORE_TIER],
                                            DOWNLINK,
                                            CORE_TIER,
                                            false,
                            ...
                            queues_nc_nup[core][agg][b] = asymmetric_alloc_queue(
                                down_queueLogger, _queue_down[CORE_TIER], DOWNLINK, CORE_TIER);
                            queues_nup_nc[agg][core][b] = asymmetric_alloc_queue(
                                up_queueLogger, _queue_up[AGG_TIER], UPLINK, AGG_TIER);
                        }
                    }

                    queues_nc_nup[core][agg][b]->setName(
                        fmt::format("CS{}->US{}({})", core, agg, b));
                    queues_nup_nc[agg][core][b]->setName(
                        fmt::format("US{}->CS{}({})", agg, core, b));
                    ...
                    simtime_picosec hop_latency =
                        (_hop_latency == 0) ? _link_latencies[CORE_TIER] : _hop_latency;
                    pipes_nup_nc[agg][core][b] = new Pipe(hop_latency, *_eventlist);
                    pipes_nup_nc[agg][core][b]->setName(
                        fmt::format("Pipe-US{}->CS{}({})", agg, core, b));
                    ...
                    pipes_nc_nup[core][agg][b]->setName(
                        fmt::format("Pipe-CS{}->US{}({})", core, agg, b));
                }
            }
        }
    }
...
vector<const Route*>* FatTreeTopology::get_bidir_paths(uint32_t src, uint32_t dest, bool reverse) {
    ...
    // Queue* pqueue = new Queue(_linkspeed, memFromPkt(FEEDER_BUFFER), *_eventlist,
    // simplequeuelogger); pqueue->setName("PQueue_" + ntoa(src) + "_" + ntoa(dest));
    ...
    if (HOST_POD_SWITCH(src) == HOST_POD_SWITCH(dest)) {
        ...
        if (reverse) {
            ...
            if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                routeback->push_back(
                    queues_ns_nlp[dest][HOST_POD_SWITCH(dest)][0]->getRemoteEndpoint());
        ...
        // there are K/2 paths between the source and the destination  <- this is no longer true for
        // bundles
        if (_tiers == 2) {
        ...
        for (uint32_t upper = MIN_POD_AGG_SWITCH(pod); upper <= MAX_POD_AGG_SWITCH(pod); upper++) {
            for (uint32_t b_up = 0; b_up < _bundlesize[AGG_TIER]; b_up++) {
                for (uint32_t b_down = 0; b_down < _bundlesize[AGG_TIER]; b_down++) {
                    // b_up is link number in upgoing bundle, b_down is link number in downgoing
                    // bundle note: no bundling supported between host and tor - just use link
                    // number 0
                    ...
                    if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                        routeout->push_back(
                            queues_ns_nlp[src][HOST_POD_SWITCH(src)][0]->getRemoteEndpoint());
                    ...
                    if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                        routeout->push_back(
                            queues_nlp_nup[HOST_POD_SWITCH(src)][upper][b_up]->getRemoteEndpoint());
                    ...
                    if (reverse) {
                        ...
                        if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                            routeback->push_back(
                                queues_nlp_nup[HOST_POD_SWITCH(dest)][upper][b_down]
                                    ->getRemoteEndpoint());
            ...
            for (uint32_t l = 0; l < _radix_up[AGG_TIER] / _bundlesize[CORE_TIER]; l++) {
                ...
                for (uint32_t b1_up = 0; b1_up < _bundlesize[AGG_TIER]; b1_up++) {
                    for (uint32_t b1_down = 0; b1_down < _bundlesize[AGG_TIER]; b1_down++) {
                        // b1_up is link number in upgoing bundle from tor to agg, b1_down is link
                        // number in downgoing bundle

                        for (uint32_t b2_up = 0; b2_up < _bundlesize[CORE_TIER]; b2_up++) {
                            for (uint32_t b2_down = 0; b2_down < _bundlesize[CORE_TIER];
                                 b2_down++) {
                                // b2_up is link number in upgoing bundle from agg to core, b2_down
                                // is link number in downgoing bundle note: no bundling supported
                                // between host and tor - just use link number 0
                                // upper is nup
                                ...
                                routeout->push_back(
                                    queues_nlp_nup[HOST_POD_SWITCH(src)][upper][b1_up]);
                                routeout->push_back(
                                    pipes_nlp_nup[HOST_POD_SWITCH(src)][upper][b1_up]);
                                ...
                                if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                                    routeout->push_back(
                                        queues_nup_nc[upper][core][b2_up]->getRemoteEndpoint());

                                // now take the only link down to the destination server!

                                uint32_t upper2 = MIN_POD_AGG_SWITCH(HOST_POD(dest)) +
                                                  core % _agg_switches_per_pod;
                                ...
                                if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                                    routeout->push_back(
                                        queues_nc_nup[core][upper2][b2_down]->getRemoteEndpoint());

                                routeout->push_back(
                                    queues_nup_nlp[upper2][HOST_POD_SWITCH(dest)][b1_down]);
                                routeout->push_back(
                                    pipes_nup_nlp[upper2][HOST_POD_SWITCH(dest)][b1_down]);
                                ...
                                if (reverse) {
                                    ...
                                    routeback->push_back(
                                        queues_ns_nlp[dest][HOST_POD_SWITCH(dest)][0]);
                                    routeback->push_back(
                                        pipes_ns_nlp[dest][HOST_POD_SWITCH(dest)][0]);
                                    ...
                                    routeback->push_back(
                                        queues_nlp_nup[HOST_POD_SWITCH(dest)][upper2][b1_down]);
                                    routeback->push_back(
                                        pipes_nlp_nup[HOST_POD_SWITCH(dest)][upper2][b1_down]);
                                    ...
                                    if (_qt == LOSSLESS_INPUT || _qt == LOSSLESS_INPUT_ECN)
                                        routeback->push_back(
                                            queues_nc_nup[core][upper][b2_up]->getRemoteEndpoint());

                                    routeback->push_back(
                                        queues_nup_nlp[upper][HOST_POD_SWITCH(src)][b1_up]);
                                    routeback->push_back(
                                        pipes_nup_nlp[upper][HOST_POD_SWITCH(src)][b1_up]);
                                    ...
                                    routeback->push_back(
                                        queues_nlp_ns[HOST_POD_SWITCH(src)][src][0]);
                                    routeback->push_back(
                                        pipes_nlp_ns[HOST_POD_SWITCH(src)][src][0]);
...
void FatTreeTopology::print_path(std::ofstream& paths, uint32_t src, const Route* route) {
    ...
    if (route->size() / 2 == 2) {
        paths << "LS" << find_lp_switch((Queue*)route->at(1)) << " ";
        paths << "DST" << find_destination((Queue*)route->at(3)) << " ";
    } else if (route->size() / 2 == 4) {
        paths << "LS" << find_lp_switch((Queue*)route->at(1)) << " ";
        paths << "US" << find_up_switch((Queue*)route->at(3)) << " ";
        paths << "LS" << find_lp_switch((Queue*)route->at(5)) << " ";
        paths << "DST" << find_destination((Queue*)route->at(7)) << " ";
    } else if (route->size() / 2 == 6) {
        paths << "LS" << find_lp_switch((Queue*)route->at(1)) << " ";
        paths << "US" << find_up_switch((Queue*)route->at(3)) << " ";
        paths << "CS" << find_core_switch((Queue*)route->at(5)) << " ";
        paths << "US" << find_up_switch((Queue*)route->at(7)) << " ";
        paths << "LS" << find_lp_switch((Queue*)route->at(9)) << " ";
        paths << "DST" << find_destination((Queue*)route->at(11)) << " ";
    } else {
        paths << "Wrong hop count " << fmt::format("{}", route->size() / 2);
    }
...
simtime_picosec FatTreeTopology::get_two_point_diameter_latency(int src, int dst) {
    ...
    if (_tiers == 2) {
        if (HOST_POD_SWITCH(src) != HOST_POD_SWITCH(dst)) {
            ...
            if (_link_latencies[TOR_TIER] == 0) {
                ...
                diameter_latency_end_point =
                    2 * _link_latencies[TOR_TIER] + _switch_latencies[TOR_TIER];
            }
        }
    } else if (_tiers == 3) {
        if (HOST_POD_SWITCH(src) == HOST_POD_SWITCH(dst)) {
            ...
            if (_link_latencies[TOR_TIER] == 0) {
                ...
                diameter_latency_end_point =
                    2 * _link_latencies[TOR_TIER] + 2 * _switch_latencies[TOR_TIER] +
    ...
    // cout << " _tiers " << _tiers <<  " HOST_POD_SWITCH src " << HOST_POD_SWITCH(src) << " dst "
    // << HOST_POD_SWITCH(dst) << " diameter_latency_end_point " << diameter_latency_end_point<<
    // endl;
