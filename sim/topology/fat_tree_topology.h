...
public:
    ...
    enum AsymmetricMode { SYM, SINGLE, ALL };
    ...
    // For regular topologies, just use the constructor.  For custom topologies, load from a config
    // file.
    static FatTreeTopology* load(const char*         filename,
    ...
    FatTreeTopology(uint32_t            no_of_nodes,
    ...
    static void set_tier_parameters(int           tier,
                                    int           radix_up,
                                    int           radix_down,
                                    mem_b         queue_up,
                                    mem_b         queue_down,
                                    int           bundlesize,
                                    linkspeed_bps downlink_speed,
                                    int           oversub);

    static void set_ecn_parameters(bool  enable_ecn,
                                   bool  enable_on_tor_downlink,
                                   mem_b ecn_low,
                                   mem_b ecn_high) {
    ...
    static void set_failed_links(int num_failed_links) { _num_failed_links = num_failed_links; }
    static void set_failed_link_ratio(double ratio) { _failed_link_ratio = ratio; }
    static void set_failed_bidirectional(bool enabled) { _failed_bidirectional = enabled; }

    void                          init_network();
    virtual vector<const Route*>* get_bidir_paths(uint32_t src, uint32_t dest, bool reverse);

    BaseQueue* alloc_src_queue(QueueLogger* q, linkspeed_bps linkspeed);
    BaseQueue* alloc_queue(
        QueueLogger* q, mem_b queuesize, link_direction dir, int switch_tier, bool tor);
    BaseQueue* asymmetric_alloc_queue(
        QueueLogger* q, mem_b queuesize, link_direction dir, int switch_tier, bool tor);
    BaseQueue* alloc_queue(QueueLogger*   q,
                           linkspeed_bps  speed,
                           mem_b          queuesize,
                           link_direction dir,
                           int            switch_tier,
    ...
    static void set_latencies(simtime_picosec src_lp,
                              simtime_picosec lp_up,
                              simtime_picosec up_cs,
                              simtime_picosec lp_switch,
                              simtime_picosec up_switch,
                              simtime_picosec core_switch) {
    ...
    static int _num_to_fail;

    uint32_t no_of_tier_switches(int tier) const {
        if (tier == 0) {
            return NTOR;
        } else if (tier == 1) {
            return NAGG;
            ...
            throw std::invalid_argument(fmt::format("Invalid tier number: {}", tier));
        }
    }
    ...
    int get_oversubscription_ratio() {
        int ratio = _oversub[TOR_TIER];
        if (_tiers > 2)
            ratio *= _oversub[AGG_TIER];
        return ratio;
    }

    // For each tier, get its latency. Starts from lower tier, and separate tiers with ":"
    std::string get_tiers_latency() {
        std::string per_tier_latency;
        for (uint32_t i = 0; i < _tiers; i++) {
            per_tier_latency += std::to_string((uint64_t)timeAsNs(_link_latencies[i]));
            if (i < _tiers - 1) {
                per_tier_latency += ":";
            }
        }
        return per_tier_latency;
    }
    ...
    AsymmetricMode asymmetric_mode(int tier) { return _asymmetric_mode[tier]; }

    linkspeed_bps asymmetric_downlink_speed(int tier) { return _asymmetric_downlink_speeds[tier]; }

    uint32_t asymmetric_number_of_switches() { return _asymmetric_number_of_switches; }

private:
    map<Queue*, int>        _link_usage;
    static FatTreeTopology* load(istream&            file,
                                 QueueLoggerFactory* logger_factory,
                                 EventList&          eventlist,
                                 mem_b               queuesize,
                                 queue_type          q_type,
    ...
    static linkspeed_bps  _asymmetric_downlink_speeds[3];
    static AsymmetricMode _asymmetric_mode[3];
    static uint32_t       _single_asymmetric_link[3];
    static uint32_t       _asymmetric_number_of_switches;

    // degree of oversubscription at tier.  Eg _oversub[TOR_TIER] = 3 implies 3x more bw to hosts
    // than to agg switches.
    static uint32_t _oversub[3];

    // switch radix used.  Eg _radix_down[0] = 32 indicates 32 downlinks from ToRs.  _radix_up[2]
    // should be zero in a 3-tier topology.
    static uint32_t _radix_down[3];
    static uint32_t _radix_up[2];

    // switch queue size used.  Eg _queue_down[0] = 32 indicates 32 downlinks from ToRs.
    // _queue_up[2] should be zero in a 3-tier topology.
    ...
    static bool _failed_bidirectional;
    int total_tor_uplinks;
