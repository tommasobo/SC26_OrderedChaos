...
bool     FatTreeSwitch::_low_priority_trim        = false;
bool     FatTreeSwitch::_no_droping_low_header    = false;
double   FatTreeSwitch::_switch_random_drop_prob  = 0;
bool     FatTreeSwitch::_log_subflow_routes       = false;

FatTreeSwitch::FatTreeSwitch(EventList&       eventlist,
                             string           s,
                             switch_type      t,
                             uint32_t         id,
...
uint32_t FatTreeSwitch::adaptive_route_p2c(vector<FibEntry*>* ecmp_set,
...
uint32_t FatTreeSwitch::adaptive_route(vector<FibEntry*>* ecmp_set,
                                       int8_t (*cmp)(FibEntry*, FibEntry*)) {
    uint32_t choice = 0;
...
uint32_t FatTreeSwitch::replace_worst_choice(vector<FibEntry*>* ecmp_set,
                                             int8_t (*cmp)(FibEntry*, FibEntry*),
                                             uint32_t my_choice) {
    ...
    // might need to play with different alternatives here, compare to worst rather than just to
    // worst index.
...
Route* FatTreeSwitch::getNextHop(Packet& pkt, BaseQueue* ingress_port) {
    ...
    if (available_hops) {
        ...
        if (available_hops->size() > 1)
            switch (_strategy) {
                ...
                case ADAPTIVE_ROUTING:
                    if (pkt.size() < 100) {
                        // don't bother adaptive routing the small packets - don't want to pollute
                        // the tables
                        ecmp_choice = freeBSDHash(pkt.flow_id(), pkt.pathid(), _hash_salt) %
                                      available_hops->size();
                        break;
                    ...
                    if (_ar_sticky == FatTreeSwitch::PER_PACKET) {
                        ...
                        if (_flowlet_maps.find(pkt.flow_id()) != _flowlet_maps.end()) {
                            ...
                            // only reroute an existing flow if its inter packet time is larger than
                            // _sticky_delta and and 50% chance happens. and (commented out) if the
                            // switch has not taken any other placement decision that we've not seen
                            // the effects of.
                            if (EventList::now() - f->_last > _sticky_delta &&
                                ...
                                if (fn(available_hops->at(f->_egress),
                                       available_hops->at(new_route)) < 0) {
                                    f->_egress   = new_route;
                                    _last_choice = EventList::now();
                                    // cout << "Switch " << _type << ":" << _id << " choosing new
                                    // path "<<  f->_egress << " for " << pkt.flow_id() << " at " <<
        ...
        if (FatTreeSwitch::_log_subflow_routes && pkt.size() > 100) {
            printf("Switch is %s - Flow is %d - Subflow id is %d - Time is %lu\n",
                   _name.c_str(), pkt.flow_id(), pkt.pathid(),
                   static_cast<unsigned long>(EventList::now()));
        }
    ...
    if (_type == TOR) {
        if (_ft->HOST_POD_SWITCH(pkt.dst()) == _id) {
            ...
            if (_uproutes)
                ...
                for (uint32_t k = agg_min; k <= agg_max; k++) {
                    ...
                    /*
                      FatTreeSwitch* next =
                      (FatTreeSwitch*)_ft->queues_nlp_nup[_id][k]->getRemoteEndpoint(); assert
                      (next->getType()==AGG && next->getID() == k);
                    */
                }
            ...
            if (_uproutes)
                ...
                for (uint32_t l = 0; l < uplink_bundles; l++) {
                    ...
                    for (uint32_t b = 0; b < _ft->bundlesize(CORE_TIER); b++) {
                        ...
                        /*
                          FatTreeSwitch* next =
                          (FatTreeSwitch*)_ft->queues_nup_nc[_id][k]->getRemoteEndpoint(); assert
                          (next->getType()==CORE && next->getID() == k);
                        */

                        _fib->addRoute(pkt.dst(), r, 1, UP);

                        // cout << "AGG switch " << _id << " adding route to " << pkt.dst() << " via
                        // CORE " << k << " bundle_id " << b << endl;
                    }
                }
                //_uproutes = _fib->getRoutes(pkt.dst());
        ...
        uint32_t nup =
            _ft->MIN_POD_AGG_SWITCH(_ft->HOST_POD(pkt.dst())) + (_id % _ft->agg_switches_per_pod());
        for (uint32_t b = 0; b < _ft->bundlesize(CORE_TIER); b++) {
            Route* r = new Route();
            // cout << "CORE switch " << _id << " adding route to " << pkt.dst() << " via AGG " <<
            // nup << endl;
