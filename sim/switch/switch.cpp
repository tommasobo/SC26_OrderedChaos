...
void Switch::sendPause(LosslessQueue* problem, unsigned int wait) {
    cout << "Switch " << _name << " link " << problem->_name << " blocked, sending pause " << wait
         << endl;
...
void Switch::add_logger(Logfile& log, simtime_picosec sample_period) {
    ...
    for (size_t i = 0; i < _ports.size(); i++) {
        // cout << "adding logger to switch " << nodename() << " id " << get_id() << " queue " <<
        // _ports.at(i)->nodename() << " id " << _ports.at(i)->get_id() << endl;
