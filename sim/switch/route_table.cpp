...
void RouteTable::removeHostRoute(int destination, int flowid) {
    auto dest_it = _hostfib.find(destination);
    if (dest_it == _hostfib.end()) {
        return;
    }

    auto* host_routes = dest_it->second;
    auto  flow_it     = host_routes->find(flowid);
    if (flow_it == host_routes->end()) {
        return;
    }

    HostFibEntry* host_entry = flow_it->second;
    Route*        route      = host_entry->getEgressPort();
    delete route;
    delete host_entry;
    host_routes->erase(flow_it);

    if (host_routes->empty()) {
        delete host_routes;
        _hostfib.erase(dest_it);
    }
}
