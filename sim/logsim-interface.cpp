...
LogSimInterface::~LogSimInterface() {
    delete compute_events_handler;
    compute_events_handler = nullptr;
    delete null_events_handler;
    null_events_handler = nullptr;
    delete _latest_recv;
    _latest_recv = nullptr;
}
...
void LogSimInterface::execute_compute(graph_node_properties comp_elem, int size_p) {
    if (_protocolName == EQDS_PROTOCOL || _protocolName == NDP_PROTOCOL || _protocolName == UEC_PROTOCOL) {
        const uint64_t compute_time_ns =
                local_compute_time_override_ns >= 0 ? local_compute_time_override_ns : comp_elem.size;
        compute_events_handler->setCompute(compute_time_ns);
        ComputeAtlahsEvent compute_event(compute_time_ns);
...
void LogSimInterface::send_event(graph_node_properties elem) {
    ...
    // Testing New 
    SendEvent event(elem.host, elem.target, elem.size, elem.tag, htsim_api->getGlobalTimeNs());
    ...
    while(!lgs_interface->aq.empty() || new_events || lgs_interface->sends_active > 0 ||  lgs_interface->compute_started > 0) {
          ...
          /* printf("Active Queue Size %d - Top Time %lu Type %d - HTSIM Time "
            ...
            case OP_LOCOP: {
                ...
                if(nexto[elem.host][elem.proc] <= elem.time) { // local o available!
                    ...
                    if (lgs_interface->local_compute_time_override_ns >= 0) {
                      elem.size = lgs_interface->local_compute_time_override_ns;
                    } else if (elem.size == 0) { // Hack for 0 Computes
                      elem.size = 1;
                    }
      ...
      if (lgs_interface->sends_active != 0 || lgs_interface->compute_started != 0) {
          if (!unlocked_elem) {
              ...
              just_running = true;
              // A transport flow must leave either a future simulator event or
              // a completion message in aq.  Without this check, a lost end
              // trigger leaves sends_active nonzero and this outer loop spins
              // forever with an empty event list (observed in the paper-stated
              // MILC-32 RACK-TLP replay).
              if (lgs_interface->sends_active > 0 && lgs_interface->aq.empty()
                  && !lgs_interface->have_more
                  && !lgs_interface->latest_receive_updated()
                  && !EventList::hasPendingSources()) {
                fprintf(stderr,
                        "Trace replay deadlock: %d active send(s) remain after the event list drained at %lu ns\n",
                        lgs_interface->sends_active,
                        lgs_interface->htsim_api->getGlobalTimeNs());
                exit(3);
              }
