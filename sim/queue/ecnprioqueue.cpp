...
ECNPrioQueue::ECNPrioQueue(linkspeed_bps bitrate,
    ...
    _serv = Q_NONE;
    stringstream ss;
    ss << "ecnprioqueue(" << bitrate / 1000000 << "Mb/s," << maxsize_lo << "bytes_L," << maxsize_hi
       << "bytes_H)";
    _nodename = ss.str();
}
...
void ECNPrioQueue::completeService() {
    Packet* pkt;
...
void ECNPrioQueue::doNextEvent() {
    completeService();
}

ECNPrioQueue::queue_priority_t ECNPrioQueue::getPriority(Packet& pkt) {
...
void ECNPrioQueue::receivePacket(Packet& pkt) {
    ...
    if (_queuesize[prio] + pkt.size() > _maxsize[prio] ||
        ...
        // this is a droptail queue but drop randomly on the last slot to try and reduce simulator
        // phase effects
        if (_logger) {
...
mem_b ECNPrioQueue::queuesize() const {
