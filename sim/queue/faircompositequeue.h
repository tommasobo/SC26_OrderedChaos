...
/*
 * A fair queue between a composite NDP queue that transforms packets into headers when there is no
 * space and services headers with priority, and a standard DROP-TAIL queue for TCP traffic.
 */
...
public:
    FairCompositeQueue(linkspeed_bps bitrate,
                       mem_b         maxsize,
                       mem_b         maxsize2,
                       EventList&    eventlist,
                       QueueLogger*  logger);
