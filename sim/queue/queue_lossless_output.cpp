...
LosslessOutputQueue::LosslessOutputQueue(linkspeed_bps bitrate,
                                         mem_b         maxsize,
                                         EventList&    eventlist,
                                         QueueLogger*  logger)
...
void LosslessOutputQueue::receivePacket(Packet& pkt, VirtualQueue* prev) {
    ...
    // remember the virtual queue that has sent us this packet; will notify the vq once the packet
    // has left our buffer.
    assert(prev != NULL);
...
void LosslessOutputQueue::completeService() {
    ...
    // TODO(aghalayini): Add INT information adding to packet (independent of HPCC or not)
    // if (pkt->type() == HPCC) {
    ...
    // cout << timeAsMs(eventlist().now()) << " queue " << _name << " not ready but sending pkt " <<
    // pkt->type() << endl;
    // }
