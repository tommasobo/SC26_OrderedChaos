...
CompositePrioQueue::CompositePrioQueue(linkspeed_bps bitrate,
                                       mem_b         maxsize,
                                       EventList&    eventlist,
                                       QueueLogger*  logger,
                                       uint16_t      trim_size)
...
void CompositePrioQueue::beginService() {
    ...
    if (!_enqueued_high.empty()) {
        ...
        throw std::runtime_error("CompositePrioQueue: beginService: no packets in queue");
    }
}

void CompositePrioQueue::completeService() {
    Packet* pkt;
    ...
    if (_serv == QUEUE_LOW) {
        ...
        throw std::runtime_error("CompositePrioQueue: completeService: invalid service");
    }
...
void CompositePrioQueue::doNextEvent() {
    completeService();
}

void CompositePrioQueue::receivePacket(Packet& pkt) {
...
void CompositePrioQueue::enqueue_packet(Packet& pkt) {
...
void CompositePrioQueue::check_queued() {
    ...
    if (maxpath != _max_path_len_queued) {
        throw std::runtime_error(
            "CompositePrioQueue: check_queued: maxpath != _max_path_len_queued");
    }
    if (total_queued > 8) {
        throw std::runtime_error("CompositePrioQueue: check_queued: total_queued > 8");
    }
}

void CompositePrioQueue::trim_low_priority_packet(uint32_t prio) {
    ...
    for (i = _enqueued_low.begin(); i != _enqueued_low.end(); i++) {
        ...
        if ((*i)->path_len() > prio) {
            ...
            cout << "C [ " << _enqueued_low.size() << " " << _enqueued_high.size() << " ] STRIP"
                 << endl;
            cout << "Arriving: " << prio << " booted: " << booted_pkt->path_len() << " posn: " << c
                 << endl;
