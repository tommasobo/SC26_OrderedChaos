...
uint64_t BaseQueue::quantized_queuesize() {
    if (eventlist().now() - _last_update_qs > _update_period) {
        ...
        // cout << "QS " << (uint32_t)_last_qs << " queuesize " << queuesize() << " max " <<
        // maxsize() << endl;
    }
    return _last_qs;
}
...
PriorityQueue::PriorityQueue(linkspeed_bps bitrate,
                             mem_b         maxsize,
                             EventList&    eventlist,
                             QueueLogger*  logger)
...
void PriorityQueue::completeService() {
    ...
    if (queuesize() > 0) {
        if (_state_send == LosslessQueue::READY)
            ...
            // we've received pause or are already paused and will do nothing until the other end
            // unblocks us        }
        }
    } else {
        _servicing = Q_NONE;
...
FairPriorityQueue::FairPriorityQueue(linkspeed_bps bitrate,
                                     mem_b         maxsize,
                                     EventList&    eventlist,
                                     QueueLogger*  logger)
    : HostQueue(bitrate, maxsize, eventlist, logger) {
...
FairPriorityQueue::queue_priority_t FairPriorityQueue::getPriority(Packet& pkt) {
    ...
    switch (pkt.priority()) {
        ...
        case Packet::PRIO_HI:
            ...
            break;
        default:
            throw std::runtime_error("FairPriorityQueue: getPriority: invalid packet priority");
    }
    return prio;
}
...
simtime_picosec FairPriorityQueue::serviceTime(Packet& pkt) {
    queue_priority_t prio = getPriority(pkt);
    simtime_picosec  service_time;
    switch (prio) {
        case Q_LO:
            // cout << "q_lo: " << _queuesize[Q_HI] + _queuesize[Q_MID] + _queuesize[Q_LO] << " ";
            service_time = (_queuesize[Q_HI] + _queuesize[Q_MID] + _queuesize[Q_LO]) * _ps_per_byte;
            break;
        case Q_MID:
            // cout << "q_mid: " << _queuesize[Q_MID] + _queuesize[Q_LO] << " ";
            service_time = (_queuesize[Q_HI] + _queuesize[Q_MID]) * _ps_per_byte;
            break;
        case Q_HI:
            // cout << "q_hi: " << _queuesize[Q_LO] << " ";
            service_time = _queuesize[Q_HI] * _ps_per_byte;
            break;
        default:
            throw std::runtime_error("FairPriorityQueue: serviceTime: invalid packet priority");
    }
    return service_time;
}
