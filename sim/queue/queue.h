...
class BaseQueue : public EventSource, public PacketSink {
public:
    ...
    static simtime_picosec _update_period;
        bool fail_queue = false; // Set this to true to drop packets from this queue
...
protected:
    ...
    TimeSeriesMetric* _queue_enq_metric;  // Logs when a packet is enqueued
    TimeSeriesMetric* _queue_deq_metric;  // Logs when a packet is dequeued
...
public:
    ...
    FairPriorityQueue(linkspeed_bps bitrate,
                      mem_b         maxsize,
                      EventList&    eventlist,
                      QueueLogger*  logger);
    virtual void            receivePacket(Packet& pkt);
