...
/*
    Implements a basic queue as described in the Aeolus paper and supported by most existing
   switches.

    Queue behaviour:
    - high priority packets go into a higher priority queue. This is mainly for control packets.
   maxsize bytes can be buffered by this queue before packets are dropped.
    ...
    - low priority packets (or speculative packets) are admitted as long as _queuesize_low <
   speculative_threshold (which must be smaller than maxsize for this to work properly).

    - the low priority queue also marks ECN on egress based on the queuesize and ECN thresholds low
   and high. Default config has ECN off.
*/
...
AeolusQueue::AeolusQueue(
    linkspeed_bps bitrate, mem_b maxsize, mem_b specsize, EventList& eventlist, QueueLogger* logger)
...
void AeolusQueue::completeService() {
    Packet* pkt;
    if (_serv == QUEUE_LOW) {
        ...
        throw std::runtime_error("AeolusQueue: completeService: invalid service");
    }
...
void AeolusQueue::doNextEvent() {
    completeService();
}

void AeolusQueue::receivePacket(Packet& pkt) {
    ...
    if (pkt.priority() == Packet::PRIO_HI) {
        if (_queuesize_high + pkt.size() <= _maxsize) {  // admit
            ...
            cout << "Aeolus high priority queue " << str() << "size " << _queuesize_high
                 << " dropped packet " << endl;
        ...
        if ((pkt.priority() == Packet::PRIO_MID && _queuesize_low + pkt.size() < _maxsize) ||
            (pkt.priority() == Packet::PRIO_LO &&
             _queuesize_low + pkt.size() < _speculative_thresh)) {  // admit
            Packet* pkt_p = &pkt;
...
mem_b AeolusQueue::queuesize() const {
