...
bool UecSink::_pflr_receiver_trim_probe_coalescing = false;
bool UecSink::_pflr_accept_header_promoted_probes = false;
        ...
        if (_pflr_scheme_id == 0) {
            ...
            * --------------------------------------------------------------*/
            ...
            if (pkt.epsn() >= _pflr3_bitmap_start_psn) {
                ...
                /* if (pkt.flow_id() == 5822) {
                    printf("Idx is %lu Resize to %lu\n", idx, _pflr3_receive_bitmap.size());
                } */
            ...
            /* if (pkt.flow_id() == 5822) {
                cout << "Received PSN " << pkt.epsn() << " Start PSN " << _pflr3_bitmap_start_psn << " Bitmap Size " << _pflr3_receive_bitmap.size() << endl;     
                if (pkt.epsn() >= _pflr3_bitmap_start_psn) {
                    size_t idx = pkt.epsn() - _pflr3_bitmap_start_psn;
                    cout << static_cast<int>(_pflr3_receive_bitmap[idx]) << endl; // print as number
                } else {
                    cout << "(old window)" << endl;
                }
            } */
...
void UecSink::processProbe(UecDataPacket& pkt) {
    ...
    if (_pflr_receiver_trim_probe_coalescing &&
        !_pflr_accept_header_promoted_probes && pkt.header_only()) {
        // A probe is useful as loss evidence only while it preserves FIFO
        // order behind the data packet it names.  An unmodified composite
        // queue marks a probe as a header when its low-priority queue is
        // full; from that point the probe uses the high-priority queue and
        // may overtake the named data packet.  Do not turn that reordered
        // marker into a false NACK.  A later in-order probe, trim, PSN gap,
        // or retransmission timer still supplies safe loss evidence.
        return;
    }
    /* cout << timeAsUs(getSrc()->eventlist().now()) << " flow " << _src->flow()->flow_id()
    ...
    if (_pflr_scheme_id == 0) {
        ...
        if ((pkt.pflr_probe_type() == UecDataPacket::SECTION_END) ||
            ...
            uint64_t dbg_nacks_this_probe = 0;
            ...
            for (auto intermediate_psn = first_psn; intermediate_psn <= pkt.epsn();
                ...
                if (_pflr3_receive_bitmap[intermediate_psn - _pflr3_bitmap_start_psn] == 1) {
                    ...
                    pflrSendNack(intermediate_psn, pkt_ev);
                    dbg_nacks_this_probe++;

                    _pflr3_nack_bitmap[intermediate_psn - _pflr3_bitmap_start_psn] = 1;
                }
            }
            // after the loop:
            if (UecSrc::_log_reaction_events && dbg_nacks_this_probe) {
                std::cout << "[PFLR3:NACKS] us=" << timeAsUs(getSrc()->eventlist().now())
                        << " flow=" << _src->flow()->flow_id()
                        << " epsn=" << pkt.epsn()
                        << " n=" << dbg_nacks_this_probe << "\n";
            }
        } else if (pkt.pflr_probe_type() == UecDataPacket::PROACTIVE_RTX) {
            auto ep = pkt.epsn();
            if (ep >= _pflr3_bitmap_start_psn) {
                size_t idx = ep - _pflr3_bitmap_start_psn;
                if (idx >= _pflr3_receive_bitmap.size()) {
                    _pflr3_receive_bitmap.resize(idx + 1, 0);
                    _pflr3_nack_bitmap.resize(idx + 1, 0);
                }
                // A retransmission probe is fresh evidence that the packet is
                // still missing, even if an earlier transmission was NACKed.
                if (_pflr3_receive_bitmap[idx] == 0) {
                    if (UecSrc::_log_reaction_events) {
                        printf("Sending NACK3 for PSN %d (start=%d)\n", ep,
                               _pflr3_bitmap_start_psn);
                    }
                    pflrSendNack(ep, pkt.path_id());
                    _pflr3_nack_bitmap[idx] = 1;
                }
            } else {
                // old window; already processed or cumulatively acked
            }
        } else {
            cout << "pflr state error" << endl;
            abort();
...
void UecSink::processTrimmed(const UecDataPacket& pkt) {
    ...
    if (UecSrc::usePflr() && !backgroundECMPFlow && _pflr_scheme_id == 3) {
        ...
        if (_pflr_receiver_trim_probe_coalescing &&
            pkt.epsn() >= _pflr3_bitmap_start_psn) {
            // Reuse the receiver's existing per-PSN NACK bitmap. In the
            // legacy path, a trim arriving before any data/probe sends a
            // NACK without recording it; the paired proactive data probe
            // then sends the same NACK again. Recording the first NACK here
            // coalesces that pair entirely at the receiver. PROACTIVE_RTX
            // intentionally ignores this bitmap in processProbe(), so a
            // lost retransmission still receives a fresh loss signal.
            const size_t idx = pkt.epsn() - _pflr3_bitmap_start_psn;
            if (idx >= _pflr3_receive_bitmap.size()) {
                _pflr3_receive_bitmap.resize(idx + 1, 0);
                _pflr3_nack_bitmap.resize(idx + 1, 0);
            }
...
void UecSink::receivePacket(Packet& pkt, uint32_t port_num) {
    ...
    switch (pkt.type()) {
        case UECDATA: {
            auto& data_pkt = static_cast<UecDataPacket&>(pkt);
            // A trimmed probe is still a probe and retains the metadata used
            // for precise loss detection.
            if (data_pkt.is_probe_packet()) {
                processProbe(data_pkt);
            } else if (pkt.header_only() || pkt.header_low_only()) {
                processTrimmed(data_pkt);
            } else {
