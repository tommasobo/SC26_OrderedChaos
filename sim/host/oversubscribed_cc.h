...
public:
    ...
    simtime_picosec nextInterval() {
        // target feedback delay is 1.5 baseRTTs; randonmize around this value to avoid unwanted
        // synchronization between different sources.
        return (simtime_picosec)((0.75 + drand() / 2) * 1.5 * _base_rtt);
    }

    inline void ecn_received(mem_b size) {
        _ecn++;
        _ecn_bytes += size;
    }

    inline void data_received(mem_b size) {
        _received++;
