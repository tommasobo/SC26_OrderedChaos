...
OversubscribedCC::OversubscribedCC(EventList& eventList, UecPullPacer* pacer)
    : EventSource(eventList, "OversubscribedCC"), _pullPacer(pacer) {
    _rate                 = 1;
    _g                    = 0;
    _received_bytes       = 0;
    _ecn_bytes            = 0;
...
void OversubscribedCC::doNextEvent() {
    doCongestionControl();
    eventlist().sourceIsPendingRel(*this, nextInterval());
}

void OversubscribedCC::doCongestionControl() {
    ...
    if (UecSrc::_debug)
        cout << "At " << timeAsUs(eventlist().now()) << " oversubscribed cc " << _g << " rate "
             << _rate << " flow " << _pullPacer->get_id() << " total packets " << total_packets
             << " trimmed " << trimmed_other << " trimmed last hop " << trimmed_last_hop << " ecn "
             << ecn << endl;
