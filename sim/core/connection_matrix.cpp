...
ConnectionMatrix::ConnectionMatrix(uint32_t n) : N(n), conns(nullptr) {}
...
void ConnectionMatrix::setPermutation(uint32_t conn, uint32_t rack_size) {
    uint32_t             dest, pos;
    std::vector<int64_t> to(N, -1);  // enough to hold 32-bit ID and -1
    uint32_t*            rack_load;
    uint32_t             rack_count;
...
void ConnectionMatrix::setPermutationShuffle(uint32_t conns) {
    ...
    // we'll shuffle all the destinations for all possible sources, then pick the right number of
...
void ConnectionMatrix::setIncastLocalPerm(uint32_t hosts_per_hotspot) {
    ...
    if (crt > N) {
        cerr << "Topology Error: can't have a FatTree with " << N << " nodes\n";
        exit(1);
    }
    ...
    for (uint32_t dst = (first / per_rack) * per_rack; dst < ((first / per_rack) + 1) * per_rack;
         dst++) {
        if (dst == first)
            continue;
...
bool ConnectionMatrix::load(istream& file) {
    ...
    while (std::getline(file, line)) {
        ...
        } else if (tokens[0].find("->") != string::npos || tokens[0] == "trigger" ||
                   tokens[0] == "failure") {
        ...
        if (dstix != string::npos) {
            ...
            for (size_t i = 1; i < tokens.size(); i++) {
                if (tokens[i] == "start") {
                    ...
                    cerr << "Error: unknown token: " << tokens[i] << " at line " << linecount
                         << endl;
                    exit(1);
                }
            }
            ...
            for (size_t i = 1; i < tokens.size(); i++) {
                if (tokens[i] == "switch_type") {
                    ...
                    cerr << "Error: unknown failure attribute " << tokens[i] << " at line "
                         << linecount << endl;
                    exit(1);
                }
            }
    ...
    if (conn_count != conns_size) {
        cerr << "Mismatch in connection count, specified: " << conns_size << " actual "
             << conn_count << endl;
        exit(1);
    }
    if (trig_count != triggers_size) {
        cerr << "Mismatch in trigger count, specified: " << triggers_size << " actual "
             << trig_count << endl;
        exit(1);
    }
    if (failures.size() != failures_size) {
        cerr << "Mismatch in failure count, specified: " << failures_size << " actual "
             << failures.size() << endl;
        exit(1);
    }
...
Trigger* ConnectionMatrix::getTrigger(triggerid_t id, EventList& eventlist) {
