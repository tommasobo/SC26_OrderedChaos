...
void Trigger::add_target(TriggerTarget& target) {
...
void SingleShotTrigger::activate() {
    assert(!_done);
...
MultiShotTrigger::MultiShotTrigger(EventList& eventlist, triggerid_t id) : Trigger(eventlist, id) {
    _next = 0;
}

void MultiShotTrigger::activate() {
...
void BarrierTrigger::activate() {
    assert(_activations_remaining > 0);
    assert(_targets.size() > 0);
    _activations_remaining--;
    if (_activations_remaining > 0) {
        cout << "Trigger " << _id << " activated, activations remaining: " << _activations_remaining
             << endl;
        return;
    }
    vector<TriggerTarget*>::iterator i;
