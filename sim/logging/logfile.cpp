...
#define _CRT_SECURE_NO_DEPRECATE  // For Visual Studio: this allows the unsafe operation fopen()
...
RawLogEvent::RawLogEvent(double   time,
                         uint32_t type,
                         uint32_t id,
                         uint32_t ev,
                         double   val1,
...
string RawLogEvent::str() {
    ...
    ss << " Type=" << _type << " ID=" << _id << " EV=" << _ev << " VAL1=" << _val1
       << " VAL2=" << _val2 << " VAL3=" << _val3;
    return ss.str();
}
...
void Logfile::addLogger(Logger& logger) {
...
void Logfile::writeRecord(
    uint32_t type, uint32_t id, uint32_t ev, double val1, double val2, double val3) {
