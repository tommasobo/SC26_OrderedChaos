...
def plot_violin(
    ...
    sns.set_theme(style="whitegrid", context="talk", font="serif", font_scale=TEXT_SCALE)
...
def plot_violin_multi(
    ...
    # Orders PFLD With\nBuff. Headroom
    drop_order = [x for x in ["PFLD Without\nBuff. Headroom", "PFLD With\nBuff. Headroom"] if x in df_all["drop_group"].unique().tolist()]
    ...
    if bar:
        ...
        try:
            if len(drop_order) == 2 and len(present_algs) >= 1:
                ...
                # Aggregate with the same estimator used by seaborn
                agg = (
                ...
                # Proceed only if bar layout matches expectations
                if n_groups == 2 and n_per_group == len(present_algs):
                    for j, alg in enumerate(present_algs):
                        ...
                        y_off = 0.035 * (ax.get_ylim()[1] - ax.get_ylim()[0])
                        ax.text(x, y + y_off, f"-{reduction:.0f}%", ha="center", va="bottom", fontsize=13)
        except Exception:
            # Be robust if layout/values are unexpected
            pass
        # --- END NEW ---
    else:
        sns.violinplot(
    ...
    fig.savefig("pfld_headroom.pdf", dpi=dpi, bbox_inches="tight")
...
def _classify_log_label(stem: str):
    s = stem.lower()
    drop_group = "PFLD Without\nBuff. Headroom" if "nodrop" in s else "PFLD With\nBuff. Headroom"
    if "pfld" in s:
        alg = "PFLD"
