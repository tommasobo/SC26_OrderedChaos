...
def summarize(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for (algorithm, trim), rows in frame.groupby(["algorithm", "trim"], sort=False):
        ...
        records.append({
            ...
            "mean_rto_events": float(np.mean(values)),
            "ci95_low": low,
            "ci95_high": high,
        })
...
def draw(frame: pd.DataFrame, summary: pd.DataFrame, args: argparse.Namespace, scale: str) -> None:
    ...
    figure, axis = plt.subplots(figsize=(12.4, 3.4))
    ...
    for trim_index, trim in enumerate(("off", "on")):
        ...
        values = np.array([float(row.median_rto_events) for row in rows])
        ...
        for index, algorithm in enumerate(ORDER):
            seeds = frame[(frame.algorithm == algorithm) & (frame.trim == trim)].rto_events.to_numpy(float)
            axis.scatter(np.full(len(seeds), positions[index]) + rng.uniform(-0.045, 0.045, len(seeds)),
                         seeds, s=11, color="black", alpha=0.45, zorder=5)
            axis.text(positions[index], max(highs[index], values[index]) * 1.12,
                      compact_count(values[index]), ha="center", va="bottom",
                      fontsize=8, fontweight="bold")
    suffix = "" if scale == "log" else "_linear"
    if scale == "log":
        axis.set_yscale("log")
        axis.set_ylabel("RTO events per run (log scale)")
    else:
    ...
    axis.set_title(
        ...
        "Bars and labels: median RTO count; whiskers: 95% bootstrap CI; points: individual seeds",
        fontsize=11,
    )
